...
export class ExmanQuizComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    console.log('User data:', this.userId, this.username, this.email);
  ...
  async loadQuizzes(): Promise<void> {
    try {
      ...
      this.quizzes = this.quizzes.filter(quiz => quiz.question !== null);
      this.loading = false;
  ...
  chooseOption(option: string): void {
    ...
    this.selectedOptions[this.currentQuiz.id] = option;
    this.cdr.markForCheck();
  }
  ...
  submitQuiz(): void {
    const selectedAnswers = this.quizzes
      ...
      .map(quiz => this.selectedOptions[quiz.id]);
    ...
    this.examServiceService.completeExam(this.examId, this.userId, selectedAnswers, this.username, this.ExamTitle, )
      .subscribe({
        next: (response) => {
          ...
          this.certificatService.downloadCertificate(this.userId, this.examId,this.email);
        },
        error: (error) => {
