...
public class PartnershipService implements IPartnershipService {
    ...
    public Partnership applyForPartnership(int entrepriseId, int proposalId) {
        ...
        // 🚫 Ban check
        if (entreprise.isBanned()) {
            throw new IllegalStateException("This entreprise is banned and cannot apply to proposals.");
        }

        Proposal proposal = proposalRepository.findById(proposalId)
                .orElseThrow(() -> new EntityNotFoundException("Proposal not found with id " + proposalId));

        // 🚫 Check if the proposal is already fulfilled
    ...
    public Partnership terminatePartnership(int partnershipId) {
        Partnership partnership = partnershipRepository.findById(partnershipId)
                .orElseThrow(() -> new EntityNotFoundException("Partnership not found"));

        if (partnership.getPartnershipStatus() == PartnershipStatus.terminated) {
            throw new IllegalStateException("Partnership is already terminated");
        }

        partnership.setPartnershipStatus(PartnershipStatus.terminated);
        entrepriseService.checkAndBanEntreprise(partnership.getEntreprise().getIdEntreprise());
        // Optional: Add termination date or reason if needed
        return partnershipRepository.save(partnership);
    }
    ...
    public Partnership updatePartnershipStatus(int partnershipId, PartnershipStatus newStatus) {
        Partnership partnership = partnershipRepository.findById(partnershipId)
                .orElseThrow(() -> new RuntimeException("Partnership not found"));

        partnership.setPartnershipStatus(newStatus);
        Partnership updated = partnershipRepository.save(partnership);

        if (newStatus == PartnershipStatus.Approved && partnership.getEntreprise() != null) {
            entrepriseService.calculateAndUpdateEntrepriseScore(partnership.getEntreprise().getIdEntreprise());
        }

        return updated;
    }
    public List<Partnership> getPartnershipsByEntrepriseId(int entrepriseId) {
        return partnershipRepository.getPartnershipsByEntrepriseId(entrepriseId);
    }

    public List<Partnership> getPartnershipsByPartnerId(int partnerId) {
        return partnershipRepository.findByProposalEntreprisePartnerId(partnerId);
    }
}
